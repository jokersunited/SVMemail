...
def retrain_model(email_df, size):
    Train_X, Test_X, Train_Y, Test_Y = model_selection.train_test_split(email_df['clean_text'], email_df['label'], test_size=size)
    ...
    pickle.dump({'model': SVM, 'report': report, 'cfm': cfm, 'accuracy': accuracy, 'size': size}, open(filename, 'wb'))
